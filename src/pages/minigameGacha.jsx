...
const MinigameGacha = () => {
  ...
  const gachaItems = [
    // 10 xe common
    { id: 1, image: mercedes300, name: "Mercedes-Benz 300", rarity: "common" },
    { id: 2, image: datsunFairladyZ2, name: "Datsun Fairlady 240Z", rarity: "common" },
    { id: 3, image: datsunFairladyZ, name: "Datsun Fairlady Z", rarity: "common" },
    { id: 4, image: dinkaHakumai, name: "Dinka Hakumai", rarity: "common" },
    { id: 5, image: hondaCivicEK9, name: "Honda Civic EK9", rarity: "common" },
    { id: 6, image: jb700, name: "JB 700", rarity: "common" },
    { id: 7, image: nissanSkyline, name: "Nissan Skyline 2000GT", rarity: "common" },
    { id: 8, image: stanierTaxi, name: "Stanier Taxi", rarity: "common" },
    { id: 9, image: tofasSerge, name: "Tofaş Serçe", rarity: "common" },
    { id: 10, image: volkswagenGol, name: "Volkswagen Gol GL", rarity: "common" },
    
    // 5 xe rare
    { id: 11, image: audiQ5, name: "Audi Q5 Quattro SLine", rarity: "rare" },
    { id: 12, image: kiaStinger, name: "Kia Stinger 2022", rarity: "rare" },
    { id: 13, image: suzukiLiana, name: "Suzuki Liana GLX", rarity: "rare" },
    { id: 14, image: toyotaLandCruiser, name: "Toyota Land Cruiser", rarity: "rare" },
    { id: 15, image: toyotaTundra, name: "Toyota Tundra TRD Pro", rarity: "rare" },
    
    // 4 xe epic
    { id: 16, image: fordMustangHoonicorn, name: "Ford Mustang Hoonicorn", rarity: "epic" },
    { id: 17, image: koenigseggJesko, name: "Koenigsegg Jesko Attack", rarity: "epic" },
    { id: 18, image: lampadatiVayra, name: "Lampadati Vayra", rarity: "epic" },
    { id: 19, image: lightningMcQueen, name: "Lightning McQueen", rarity: "epic" },
    
    // 3 xe legendary
    { id: 20, image: astonMartinValhalla, name: "Aston Martin Valhalla", rarity: "legendary" },
    { id: 21, image: koenigseggRegera, name: "Koenigsegg Regera", rarity: "legendary" },
    { id: 22, image: modLucky, name: "MOD Lucky", rarity: "legendary" },
  ];
  ...
  const getRarityTextColor = (rarity) => {
    switch (rarity) {
      case 'common': return 'text-blue-600';
  ...
  const getRarityAnimation = (rarity) => {
    switch (rarity) {
      case 'common': return 'animate-pulse-slow';
      case 'rare': return 'animate-pulse-slow animate-bounce-gentle';
      case 'epic': return 'animate-pulse-slow animate-glow-epic';
      case 'legendary': return 'animate-pulse-slow animate-glow-legendary animate-float';
  ...
  const startSpin = () => {
    ...
    const newShuffledItems = shuffleArray(gachaItems);
    ...
    setTimeout(() => {
      ...
      const randomSpins = Math.floor(Math.random() * 5) + 10; // 10-14 vòng để quay nhanh
      
      // Chọn item thắng cuộc ngẫu nhiên từ danh sách đã xáo trộn
      const wonItemIndex = Math.floor(Math.random() * newShuffledItems.length);
      const wonItem = newShuffledItems[wonItemIndex];
      ...
      const totalItemsToScroll = randomSpins * newShuffledItems.length + wonItemIndex;
      
      // Offset cuối = -(tổng khoảng cách - điều chỉnh để center item khớp với vạch đỏ)
      const rawOffset = totalItemsToScroll * itemWidth;
      const adjustmentForCenter = containerCenter - itemCenterInItem;
      const finalOffset = -(rawOffset - adjustmentForCenter);
    ...
    <>
      ...
      <style jsx>{`
        ...
        @keyframes glow {
          ...
          50% { box-shadow: 0 0 30px rgba(255, 215, 0, 0.9), 0 0 40px rgba(255, 215, 0, 0.6); }
        }
        
        @keyframes glow-success {
          0%, 100% { box-shadow: 0 0 20px rgba(34, 197, 94, 0.6); }
          50% { box-shadow: 0 0 30px rgba(34, 197, 94, 0.9), 0 0 40px rgba(255, 215, 0, 0.6); }
        }
        ...
        @keyframes glow-epic {
          ...
          50% { box-shadow: 0 0 25px rgba(147, 51, 234, 0.8), 0 0 35px rgba(147, 51, 234, 0.4); }
        }
        
        @keyframes glow-legendary {
          0%, 100% { box-shadow: 0 0 20px rgba(249, 115, 22, 0.6); }
          50% { box-shadow: 0 0 30px rgba(249, 115, 22, 0.9), 0 0 40px rgba(255, 215, 0, 0.6); }
        }
        ...
        .animate-glow-success { animation: glow-success 1.5s ease-in-out infinite; }
        .animate-shimmer { animation: shimmer 2s ease-in-out infinite; }
        .animate-spin-slow { animation: spin-slow 3s linear infinite; }
        .animate-pulse-slow { animation: pulse-slow 2s ease-in-out infinite; }
        .animate-bounce-gentle { animation: bounce-gentle 1.5s ease-in-out infinite; }
        .animate-glow-epic { animation: glow-epic 2s ease-in-out infinite; }
        .animate-glow-legendary { animation: glow-legendary 1.5s ease-in-out infinite; }
      ...
      <div className="relative z-10 px-4 py-8">
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="text-6xl font-extrabold mb-8 text-white drop-shadow-2xl" style={{
            ...
            <div className="flex justify-center items-center gap-8 mb-8 relative">
              <img 
                ...
                className={`w-24 h-24 object-contain transition-all duration-1000 ${
                  isAnimating ? 'transform translate-x-4' : ''
                }`}
              />
              <img 
                src={keyImage} 
                ...
                className={`w-16 h-16 object-contain transition-all duration-1000 ${
                  isAnimating ? 'transform -translate-x-4 rotate-45' : ''
                }`}
              />
            </div>
            <button 
      ...
      {showSpinPopup && (
        <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 animate-fadeIn">
          <div className="bg-gray-900 rounded-xl p-6 max-w-4xl w-full mx-4 relative transform transition-all duration-500 ease-out animate-slideInUp shadow-2xl border border-gray-600">
            ...
            <h2 className="text-3xl font-bold text-white text-center mb-6 animate-pulse" style={{
            ...
            <div className="relative overflow-hidden border-4 border-yellow-400 rounded-lg mb-6 bg-gradient-to-r from-purple-900 via-blue-900 to-purple-900 h-36 shadow-inner animate-glow">
              ...
              {/* Dải items quay */}
              <div 
                className={`flex transition-transform items-center h-full ${isSpinning ? 'duration-[10000ms] ease-out' : 'duration-300'}`}
                style={{ 
              ...
              >
                ...
                {Array.from({ length: 50 }, (_, i) => (
                  <React.Fragment key={i}>
                    {(shuffledItems.length > 0 ? shuffledItems : gachaItems).map((item, index) => (
                      <div 
                        key={`${i}-${index}`}
                        className={`w-[120px] h-28 border-2 ${getRarityColor(item.rarity)} flex items-center justify-center rounded-lg flex-shrink-0 transition-all duration-300 hover:scale-105 shadow-lg`}
                      >
                        <img 
                          src={item.image} 
                          alt={item.name}
            ...
            {showCongrats && wonItem && (
              <div className="text-center animate-slideInUp">
                <div className="bg-gradient-to-r from-green-500 via-emerald-600 to-green-500 rounded-lg p-6 mb-4 shadow-2xl border-2 border-yellow-400 animate-glow-success">
                  <h3 className="text-2xl font-bold text-white mb-4 animate-bounce" style={{
                    textShadow: '0 0 20px rgba(255, 255, 255, 0.8), 0 0 40px rgba(255, 215, 0, 0.6)',
                  }}>
                    🎉 CHÚC MỪNG BẠN ĐÃ TRÚNG THƯỞNG! 🎉
                  </h3>
                  <div className={`inline-block border-4 ${getRarityColor(wonItem.rarity)} rounded-lg p-4 transform hover:scale-105 transition-all duration-300 shadow-xl ${getRarityAnimation(wonItem.rarity)}`}>
                    <img 
                      src={wonItem.image} 
                      alt={wonItem.name}
                      className="w-24 h-24 object-contain mx-auto mb-2"
                    />
                    <p className="text-lg font-semibold text-gray-800">{wonItem.name}</p>
                    <p className={`text-sm font-bold uppercase tracking-wider ${getRarityTextColor(wonItem.rarity)}`}>
                      {wonItem.rarity}
                    </p>
                  </div>
                </div>
      ...
      <audio 
        ...
        loop={false}
      >
        <source src={spinningSound} type="audio/mpeg" />
